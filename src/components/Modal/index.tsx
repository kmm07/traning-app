  ...
  return (
    <>
      ...
      <div className={`modal ${modalClassName ?? ""}`} role="dialog">
        ...
        <label htmlFor={id} className="absolute inset-0 cursor-default" aria-hidden="true" />

        <div className={`modal-box relative ${size ? sizeClass[size] : ""}`}>
          <label
            htmlFor={id}
            aria-label="إغلاق"
            /*
          ...
          {title && (
            <>
              <h3 className="text-lg font-bold text-content pe-10">{title}</h3>
